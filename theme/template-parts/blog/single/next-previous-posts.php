...
<nav class="navigation container max-lg:px-3 max-w-content mt-8"
	...
	<div class="nav-links flex <?= empty($prev_post) ? 'justify-end' : 'justify-between'; ?>">
		<?php if (!empty($prev_post)) { ?>
			<a class="nav-previous text-gray-500 cursor-pointer hover:text-white transition-all hover:lg:gap-x-3 flex gap-x-2 h-fit items-center justify-center text-center"
			   title="<?= esc_attr($prev_post->post_title); ?>"
			   href="<?= esc_url(get_permalink($prev_post->ID)); ?>"
			   rel="prev">
				<svg width="30" height="30" fill="currentColor" class="bi bi-chevron-right stroke-2 text-white"
					 viewBox="0 0 16 16">
		...
		<?php if (!empty($next_post)) { ?>
			<a class="nav-next text-gray-500 cursor-pointer hover:text-white transition-all flex gap-x-2 hover:lg:gap-x-3 h-fit items-center justify-center text-center"
			   ...
			   rel="next">
				...
				<svg width="30" height="30" fill="currentColor" class="bi bi-chevron-left text-white stroke-2"
					 viewBox="0 0 16 16">
					<path fill-rule="evenodd"
