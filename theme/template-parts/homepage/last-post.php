...
<section class="container my-10 lg:my-32">
	<div class="relative flex justify-center lg:mb-24 mb-16">
		<h3 class="lg:text-4xl text-lg text-center font-bold text-white"><?= $lastWork['title'] ?? ''; ?></h3>
		<span
			class="absolute inset-0 text-center -translate-y-full text-white/5 lg:text-8xl uppercase text-3xl"><?= $lastWork['subtitle'] ?? ''; ?></span>
	</div>

	<?php if ($query && $query->have_posts()) : ?>
		<?php while ($query->have_posts()) : $query->the_post(); ?>
			<article class="flex gap-8 max-lg:flex-wrap justify-center items-center text-white">
