<header
	...
	<div
		...
		class="absolute max-lg:bottom-8 lg:top-1/2 transition-all duration-500 xl:bottom-32 z-[1] <?= is_singular('blackdsn-portfolio') ? 'items-start gap-y-8' : 'gap-y-4'; ?> xl:translate-y-1/2 inset-x-0 lg:max-w-3/5 max-lg:px-5 flex flex-col mx-auto">
		<?php
		if (is_singular('post')):
			$categories = get_the_terms(get_the_ID(), 'category');
		endif;
		if (is_singular('blackdsn-portfolio')):
			$categories = get_the_terms(get_the_ID(), 'dsn-blackdsn-categories');
		endif;
		...
		<?php endif; ?>
		<h1 class="text-2xl <?= is_singular('blackdsn-portfolio') ? 'lg:text-4xl' : 'lg:text-5xl xl:text-6xl'; ?> mb-0 transition-all duration-500 leading-[1.3] font-normal lg:text-center text-white"><?php the_title(); ?></h1>
	</div>
</header>
