<div class="grid grid-cols-2 items-center text-white/70 gap-2 justify-center lg:p-5 lg:pb-0 text-sm mb-6">
	<?php
	$boxClass = 'flex items-center lg:gap-x-2 gap-x-3';
	$svgClass = 'bg-foreground/90 rounded-lg p-2 box-content';
	$svgSize = '18'; ?>
	<span class="<?= $boxClass; ?>">
    ...
    <a href="<?= esc_url($author_url); ?>"
	   ...
	   class="hover:text-icon transition">
	...
	<span class="<?= $boxClass; ?>">
		<svg class="bi bi-tags-fill <?= $svgClass; ?>" width="<?= $svgSize ?>" height="<?= $svgSize ?>" fill="currentColor" viewBox="0 0 16 16">
		  ...
		  <path d="M1.293 7.793A1 1 0 0 1 1 7.086V2a1 1 0 0 0-1 1v4.586a1 1 0 0 0 .293.707l7 7a1 1 0 0 0 1.414 0l.043-.043z"/>
			...
			if ($categories && !is_wp_error($categories)) : ?>
				<p class="text-sm items-center !my-0 flex gap-x-3">
					<?php foreach ($categories as $category) : ?>
						<a href="<?= get_term_link($category); ?>"
						   class="text-white/70 text-sm lg:text-xs no-underline hover:text-icon"><?= $category->name; ?></a>
					<?php endforeach; ?>
				</p>
			<?php endif; ?>
     </span>
	<span class="<?= $boxClass; ?>">
		<svg class="bi bi-stopwatch-fill <?= $svgClass; ?>" width="<?= $svgSize ?>" height="<?= $svgSize ?>" fill="currentColor"
			 viewBox="0 0 16 16">
		  <path
